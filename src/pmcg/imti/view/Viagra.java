...
public class Viagra extends WindowCrud{
	...
	public void imprimir() throws InterruptedException {
		List<HashMap<String, Object>> resumoQtd;
		if(mes != null && (mes >= 1 && mes <= 12)){
			resumoQtd = new EpdDAO().getQtdViaEpdMes(Formatacao.zeroEsquerda(mes.toString(),2) + "/" + ano.toString());
		} else {
			resumoQtd = new EpdDAO().getQtdViaEpd(ano.toString());
		}
		String labelsEixoX = "";
		String dadosVia = "";
		String dadosEpd = "";
		for(HashMap<String, Object> item : resumoQtd){
			labelsEixoX = labelsEixoX + item.get("competencia").toString() + "|";
			dadosVia = dadosVia + item.get("qtdvia").toString() + "|";
			dadosEpd = dadosEpd + item.get("qtdepd").toString() + "|";
		}
		...
		param.put("labelsLegenda", "Viabilidades|Empreendimentos");
		param.put("curva", "false");
		param.put("titulo", "Quantitativo - Viabilidades e Empreendimentos");
